    ...
    <div class="flex h-screen">
        <aside
            ...
            <div class="flex h-full flex-col">
                <div
                    class="flex h-16 items-center justify-between border-b border-gray-200 px-4 dark:border-gray-800 lg:justify-start">
                    <div class="flex items-center gap-3">
                        <div
                            class="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-blue-600 text-sm font-bold text-white">
                            L
                        </div>
                ...
                <nav x-data="{ activeMenu: @js(request()->routeIs('posts.*') ? 'posts' : (request()->routeIs('categories.*') ? 'categories' : null)) }"
                    class="flex-1 space-y-1 overflow-y-auto overflow-x-hidden whitespace-nowrap p-2">
                    <a href="{{ route('dashboard') }}"
                        class="flex items-center gap-3 rounded-md px-3 py-2 transition-colors group {{ request()->routeIs('dashboard') ? 'bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400' : 'text-gray-600 hover:bg-gray-100 hover:text-blue-600 dark:text-gray-400 dark:hover:bg-gray-800' }}">
                    ...
                    <div>
                        <button type="button" @click="activeMenu = (activeMenu === 'posts' ? null : 'posts')"
                            ...
                            <svg :class="[activeMenu === 'posts' && 'rotate-180', !open ? 'w-0 opacity-0' : 'opacity-100']"
                                class="h-4 w-4 shrink-0 transition-all duration-300 ease-in-out" fill="none"
                                stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m19 9-7 7-7-7" />
                            </svg>
                        ...
                        <div x-show="activeMenu === 'posts'" x-collapse.duration.300ms style="display: none;"
                            class="mt-1" :class="!open && '!block'">
                            <div class="grid transition-[grid-template-rows,opacity] duration-300 ease-in-out"
                                :class="open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'">
                                <div class="overflow-hidden">
                                    <a href="{{ route('posts.index') }}"
                                        class="ml-11 block rounded-md px-3 py-2 transition-transform duration-300 ease-in-out {{ request()->routeIs('posts.index') ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 hover:bg-gray-100 hover:text-blue-600 dark:text-gray-400 dark:hover:bg-gray-800' }}"
                                        :class="!open ? '-translate-x-5' : 'translate-x-0'">
                                        All Posts
                                    </a>
                    ...
                    @if (auth()->user()?->role === 'admin')
                        <a href="{{ route('users.index') }}"
                            class="flex items-center gap-3 rounded-md px-3 py-2 transition-colors group {{ request()->routeIs('users.index') ? 'bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400' : 'text-gray-600 hover:bg-gray-100 hover:text-blue-600 dark:text-gray-400 dark:hover:bg-gray-800' }}">
                            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex min-w-0 flex-1 flex-col">
            <header
                class="flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="flex items-center gap-2">
                    <button type="button"
                        ...
                        <svg x-show="dark" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    @auth
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="rounded px-3 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-100">
                                Logout
                            </button>
                        </form>
                    @endauth
